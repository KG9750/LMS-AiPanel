...
describe("MLX Server adapter", () => {
  ...
  it("stopped endpoint without process stays distinct from verified", async () => {
    ...
    expect(endpoint.properties.evidence.some((e: string) => e.includes("not reachable") || e.includes("not verified"))).toBe(true);
  });
});
...
describe("Ollama adapter", () => {
  ...
  it("an unavailable installation does not fail the adapter", async () => {
    ...
    expect(endpoint.properties.evidence.some((e: string) => e.includes("api=unreachable"))).toBe(true);
  });
