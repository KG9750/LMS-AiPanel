...
describe("host identity persistence", () => {
  ...
  it("storage reinitialization is idempotent and does not duplicate the host", async () => {
    ...
    expect(first.appliedMigrations.sort()).toEqual(["0001_initial.sql", "0002_host.sql", "0003_registry.sql", "0004_snapshots.sql", "0005_refresh.sql", "0006_metrics.sql", "0007_sessions.sql"]);
